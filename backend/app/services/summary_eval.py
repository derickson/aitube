...
async def compare_engines(item: dict[str, Any]) -> dict[str, Any]:
    ...
    haiku, _haiku_error, _haiku_code = await summarize_via_haiku(prompt, title)
    haiku_ms = round((time.perf_counter() - t0) * 1000, 1)

    t0 = time.perf_counter()
    hermes_raw, _hermes_error, _hermes_code = await run_oneshot(prompt, model=settings.hermes_model)
